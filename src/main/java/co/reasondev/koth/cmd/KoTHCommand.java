...
public class KoTHCommand {
    ...
    public static void reloadHillConfig(CommandContext args, CommandSender sender) throws CommandException {
        ConfigHandler.reloadConfig("config");
    ...
    public static void startHill(CommandContext args, CommandSender sender) throws CommandException {
        ...
        HillHandler.getHillTask().setActiveHill(HillHandler.getRandomHill());
    }
    ...
    public static void createHill(CommandContext args, CommandSender sender) throws CommandException {
        ...
        else if(!PlayerUtil.isInRegion((Player) sender, WGBukkit.getRegionManager(((Player) sender).getWorld()).getRegion(args.getString(1)))) Messaging.send(sender, "&cError! You must be in the specified Region!");
        else {
    ...
    public static void removeHill(CommandContext args, CommandSender sender) throws CommandException {
        if(!HillHandler.hillExists(args.getString(0))) Messaging.send(sender, "&cError! There is no Hill with that ID!");
            ...
            HillHandler.removeHill(args.getString(0));
    ...
    public static void setChestSpawn(CommandContext args, CommandSender sender) throws CommandException {
        ...
        else if(!HillHandler.isInHill(args.getString(0), ((Player) sender).getLocation())) Messaging.send(sender, "&cError! The chest spawn must be inside of the Hill Region!");
        else {
